...
utf8_convert_ret utf8_to_local_encoding(const char *string, 
				       ...
				       char **result)
{
	return utf8_to_enc(string, nsoption_charp(accept_charset), len, result);
}
...
fb_new_face(const char *option, const char *resname, const char *fontname)
{
        ...
        if (option != NULL) {
				...
				//filepath_sfind(respaths, buf, fontname); ARTURRO
                newf->fontfile = strdup(fontname);
        }
...
bool fb_font_init(void)
{
	...
	if (fb_face == NULL) {
        ...
        if (nsoption_charp(fb_font_monochrome) == true)
